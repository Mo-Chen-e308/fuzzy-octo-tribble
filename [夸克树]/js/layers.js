...
var mod_2 = (p.div(2).floor().eq(p.div(2)))
var mod_3 = (p.div(3).floor().eq(p.div(3)))
var x=(hasUpgrade("q",32)?n(1.1):n(1))
var xx=(hasUpgrade("q",33)?n(1.075):n(1))
if(hasUpgrade("q",41)) pointscap=pointscap.add(5)
if(hasUpgrade("q",13)) pointscap=pointscap.mul(player.q.points.pow(0.1))
if(hasUpgrade("q",21)){
	if(mod_2)pointscap = pointscap.add(n(20).pow(x))
	if(!mod_2 && !hasAchievement("T",15))pointscap = pointscap.add(0)
	if(!mod_2 && hasAchievement("T",15) && !hasUpgrade("q",25))pointscap = pointscap.add(n(5).pow(x))
	if(!mod_2 && hasUpgrade("q",25))pointscap = pointscap.add(n(20).pow(x))
}
if(hasUpgrade("q",22)){
	if(mod_3)pointscap = pointscap.add(n(30).pow(x))
	if(!mod_3)pointscap = pointscap.add(0)
	if(!mod_3 && hasUpgrade("q",25)) pointscap = pointscap.add(n(30).pow(x))
}
if(hasUpgrade("q",31))pointscap = pointscap.max(n(50).pow(xx))
...
achievements: {
		...
		34: {
            ...
            tooltip: "点数超过150点<br>奖励:点数获取增加75%",
            unlocked() { return hasAchievement("T",25) },
			},
		35: {
            name: "<h3>升级堆叠Ⅳ",
		...
		41: {
            ...
            tooltip: "拥有一个及以上中微子<br>奖励：你的中微子增强你的夸克上限和点数秒产",
            unlocked() { return hasAchievement("T",35) },
			},
		42: {
            name: "<h3>微子?",
            done() {return player.n.points.gte(3)},
            tooltip: "拥有3点以上中微子<br>奖励：立体坐标系xyz点数不再重置",
            unlocked() { return hasAchievement("T",35) },
			},
		43: {
            name: "<h3>子?",
            done() {return player.n.points.gte(10)},
            tooltip: "拥有10点以上中微子<br>奖励：夸克获取增加100%",
            unlocked() { return hasAchievement("T",35) },
			},
		/*44: {
            name: "<h3>?",
            done() {return player.n.points.gte(200)},
            tooltip: "拥有20万以上夸克<br>奖励：立体坐标系xyz点数不再重置",
            unlocked() { return hasAchievement("T",35) },
			},
		45: {
            name: "<h3>夸克堆叠Ⅴ",
            done() {return player.q.points.gte(1e30)},
            tooltip: "夸克到达1e6<br>奖励：立体坐标系xyz点数不再重置",
            unlocked() { return hasAchievement("T",35) },
			},*/
},
...
addLayer("q", {
    ...
    gainMult() { 
        mult = new ExpantaNum(1)
			if(hasUpgrade("q",12)&&!hasUpgrade("q",43)) mult = mult.mul( player.q.points.pow(0.25) )
			if(hasUpgrade("q",12)&& hasUpgrade("q",43)) mult = mult.mul( player.q.points.pow(0.25).mul(2.5) )
			if(hasAchievement("T",22)) mult = mult.mul(1.5)
			if(hasUpgrade("q",15)) mult = mult.mul(1.25)
			if(hasUpgrade("q",35)) mult = mult.mul(1.15)
			if(hasAchievement("T",43)) mult = mult.mul(2)
				mult = mult.max(1)
        return mult
    },
    gainExp() {
...
doReset(resettingLayer) {
        ...
        if (layers[resettingLayer].row > this.row) {
            layerDataReset(this.layer,keep)	
		if (hasMilestone("n",0)) player.q.upgrades=[25]
		if (hasAchievement("T",42)) player.q.x = player.q.y = player.q.z = n(1000),player.q.upgrades = [24,25,34,44]
		if (hasMilestone("n",1)) player.q.upgrades=[15,24,25,34,35,44,45]
		if (hasMilestone("n",2)) player.q.upgrades=[11,12,13,14,15,21,22,23,24,25,31,32,33,34,35,41,42,43,44,45,51,52,53,54,55]
		}
	},
update(diff) {
	if(hasUpgrade("q",24)) player.q.x = player.q.x.add( new ExpantaNum(1.5).mul(diff) )
	if(hasUpgrade("q",34)) player.q.y = player.q.y.add( new ExpantaNum(1).mul(diff) )
	if(hasUpgrade("q",44)) player.q.z = player.q.z.add( new ExpantaNum(0.75).mul(diff) )
...
update(diff) {
	...
	if(player.n.points.gt(0))quarkcap = quarkcap.mul(n(1.1).pow(player.n.total))
...
milestones: {
	0: {requirementDescription: "2 中微子",
		done() {return player.n.points.gte(2)}, 
		effectDescription: "获得中微子时不再重置 Q节点'恒定不变'",
	},
	1: {requirementDescription: "5 中微子",
		done() {return player.n.points.gte(5)}, 
		effectDescription: "获得中微子时不再重置 Q节点第5列升级",
	},
	2: {requirementDescription: "8 中微子",
		done() {return player.n.points.gte(8)}, 
		effectDescription: "获得中微子时不再重置 Q节点全部升级",
	},
},
...
tabFormat: [
			...
			"display-text",function(){
				if(hasAchievement("T",41)) return "你总共拥有 " + player.n.total + " 中微子<br>" + "这使你的点数硬上限变为 " + format(new ExpantaNum(1.1).pow(player.n.total),2) + " 倍(最后计算)<br>" + "这使你的秒点数获取变为 " + format(n(1.15).pow(player.n.total),2) + " 倍  (最后计算)"
									}
			],"blank",
            "prestige-button",//获取重置点按钮
			//"resource-display",//你有xxx什么
